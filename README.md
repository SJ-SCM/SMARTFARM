...
To start the program, download the entire content of the repository as a compressed file. Then extract the contents on your local computer and launch the calculator with the main file. The information of the solar radiation variable has a slight error, follow the calculation method below instead of in the program, where it is expressed in joules instead of megajoules.
